...
Minutes = 3
Seconds = 0  # timer (win condition) was very low for testing purposes
TimeElapsed = 0
...
def draw_bg():
    for x in range(3):
        speed = 0.5
        for i in bg_images:
            screen.blit(i, ((x * WIDTH) - (Scroll * speed), 0))  #moves background images based on scroll variable + distance from player
            speed += 1
    ...
    def draw_player(self):
        ...
        if self.velocity[0] == 0:
            ...
            if Collision == True:
                ...
                if SFX_Counter >= 26:  # arbitrary number (footstep sound speed)
                    Step_SFX.play()
                    SFX_Counter = 0
